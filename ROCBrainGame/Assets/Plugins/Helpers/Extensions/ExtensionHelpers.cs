...
public class ExtensionHelpers
{
    ...
    public static IEnumerator LerpTransformRotations(Transform currentTransform, Vector3 targetRotation, float speed)
    {
        if (currentTransform == null) yield return null;
        //The Time
        float time = Time.time;
        ...
        //old position
        Vector3 transformOldRotation = currentTransform.eulerAngles;

        //While loop
        while (currentTransform.eulerAngles != targetRotation)
        {
            try
            {
                if (currentTransform != null)
                    currentTransform.eulerAngles = Vector3.Lerp(transformOldRotation, targetRotation, (Time.time - time) * speed);
                else
                    break;
            } catch (MissingReferenceException)
            {
